...
def pformat_results(pyomo_instance, pyomo_result, options):
    ...
    con_info = list()
    epsilon = 1e-6   # threshold for "so small it's zero"
        ...
        for s in m.time_season:
            for d in m.time_of_day:
                ...
                if abs(val_out) < epsilon:
                    continue
                svars['V_FlowOut'][r, p, s, d, i, t, v, o] = val_out
                svars['V_FlowIn'][r, p, s, d, i, t, v, o] = val_out / \
    ...
    if hasattr(options, 'file_location') and os.path.join('temoa_model', 'config_sample_myopic') not in options.file_location:
        for r, p, s, d, i, t, v, o in m.V_FlowOut:
            if (r, t, v) in m.LandPerActivitywoPeriod.sparse_iterkeys():
                ...
                val2 = sum(
                            value(m.V_FlowOut[r, S_p, S_s, S_d, S_i, t, v, S_o])
                            for S_p in m.time_optimize if (S_p < v + value(m.LifetimeProcess[r, t, v])) and(S_p >= v)
                            for S_s in m.time_season
                            for S_d in m.time_of_day
                            for S_i in m.processInputs[r, S_p, t, v]
                            for S_o in m.ProcessOutputsByInput[r, S_p, t, v, S_i]                            
                            )
