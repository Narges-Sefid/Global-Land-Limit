...
def db_2_dat(ifile, ofile, options):
    ...
    def query_table(t_properties, f):
        ...
        if t_index == 0:  # make sure that units and descriptions are commented out in DAT file
            ...
            for line in cur:
                ...
                search_afcom = re.search(r'^\W+$', str(after_comments))
                if not search_afcom:
    ...
    table_list = [
        ...
        ['param', 'CostVariableVariable',      '',                    '',             4],
        ['param', 'CostEmissions',             '',                    '',             3],
        ['param', 'OutputBasedStandard',       '',                    '',             6],
    ...
    with open(ofile, 'w') as f:
        ...
        if options.myopic:
            cur.execute("DELETE FROM Output_CapacityByPeriodAndTech WHERE scenario=" +
                        "'"+str(options.scenario)+"'")
            cur.execute("DELETE FROM Output_Emissions WHERE scenario=" +
                        "'"+str(options.scenario)+"'")
            cur.execute("DELETE FROM Output_Costs WHERE scenario="+"'"+str(options.scenario)+"'")
            cur.execute("DELETE FROM Output_Objective WHERE scenario=" +
                        "'"+str(options.scenario)+"'")
            cur.execute("DELETE FROM Output_VFlow_In WHERE scenario="+"'"+str(options.scenario)+"'")
            cur.execute("DELETE FROM Output_VFlow_Out WHERE scenario=" +
                        "'"+str(options.scenario)+"'")
            cur.execute("DELETE FROM Output_V_Capacity WHERE scenario=" +
                        "'"+str(options.scenario)+"'")
            cur.execute("DELETE FROM Output_Curtailment WHERE scenario=" +
                        "'"+str(options.scenario)+"'")
...
class TemoaConfig(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.path_to_data = re.sub('temoa_model$', 'data_files', dirname(abspath(__file__)))
        # Path to where debug logs will be generated for each run. By default in debug_logs folder in db_io.
        self.path_to_logs = self.path_to_data+sep+"debug_logs"
    ...
    def t_ANY_error(self, t):
        if not self.__error:
            self.__error.append({'line': [t.lineno, t.lineno], 'index': [
                                t.lexpos, t.lexpos], 'value': t.value[0]})
