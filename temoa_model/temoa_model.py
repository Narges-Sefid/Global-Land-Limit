...
def temoa_create_model(name="Temoa"):
  ...
  """
    ...
    M.Landcap_rtv = Set(dimen=3, initialize=LandCapIndices)
    M.LandPerCapacitywoPeriod = Param(M.Landcap_rtv, mutable=True, default=0.0)
    
    # Define flow out without day and season
    M.NewFlow_rpitvo = Set(dimen=6, initialize=NewFlowOutIndices)
    M.FlowOutwoDnS = Param(M.NewFlow_rpitvo, mutable=True)
    
    
    
    # Define landActivity factors
    M.Land_rtv = Set(dimen=3, initialize=LandActIndices)
    M.LandPerActivitywoPeriod = Param(M.Land_rtv, mutable=True, default=0.0)
    ...
    M.LandLimit = Param(M.RegionalGlobalIndices, M.time_optimize)
    ...
    M.LandLimitConstraint_rp = Set(
        dimen=2, initialize=lambda M: M.LandLimit.sparse_iterkeys()
    )
    M.LandLimitConstraint = Constraint(
        M.LandLimitConstraint_rp, rule=LandLimit_Constraint
    )
